...
export const templateRouter = createTRPCRouter({
  ...
  updateResum: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        const { id, sections, metadata, basics, resumeId } = input;
    ...
    .query(async ({ ctx }) => {
      ...
      try { 
        ...
        Object.values(templateMap).map((item) => {
          let temp: Resume = cloneDeep(defaultResumeState);
  ...
  getResumeById: publicProcedure
    ...
    getResumesByResumeId: protectedProcedure
    .input(z.object({
      resumeId: z.string(),
    }))
    .query(async ({ input, ctx }) => {
